...
export default function MethodBackgroundControls({
    ...
    const featuredImageSizeOptions = useMemo(() => {
        if (!imageSizes) return [];
        return imageSizes.map((size) => {
            const dims = imageDimensions[size.slug];
            const isCropped = dims?.crop;
            return {
                key: size.slug,
                name: `${size.name}${!isCropped ? ' (Uncropped)' : ''}`,
            };
        });
    }, [imageSizes, imageDimensions]);

    const hasImage = !!attributes.bgImg?.full?.url || !!attributes.useFeaturedImage;
    ...
    let jarallaxIsEnabled = attributes?.useParallax;
    if (!!attributes?.bgVideo) {
        jarallaxIsEnabled = true;
    }
    ...
    return (
        <Fragment>
            ...
            {breakpoint === 'base' && !!includeFeaturedImage && (
                <PanelRow>
                    <div style={{ width: '100%', marginBottom: '12px' }}>
                        <CheckboxControl
                            ...
                            onChange={(value) => {
                                setAttributes({
                                    useFeaturedImage: value,
                                    bgImg: { id: '' },
                                });
                                if (value) {
                                    updateSettings('featuredImageSize', 'large');
                                }
                            }}
                        />
                    </div>
            ...
            {breakpoint === 'base' && !attributes.useFeaturedImage && (
                <PanelRow>
                    <div style={{ width: '100%' }}>
                        <MediaUpload
                            onSelect={onSelect}
            ...
            {!!attributes.useFeaturedImage && !!isEnabled && (
                <PanelRow>
                    <div style={{ width: '100%' }}>
                        <CustomSelectControl
                            __next40pxDefaultSize
                            label="Preferred Image Size"
                            options={featuredImageSizeOptions}
                            value={featuredImageSizeOptions.find(opt => opt.key === settings.featuredImageSize)}
                            onChange={(option) =>
                                updateSettings('featuredImageSize', option?.selectedItem?.key)
                            }
                        />
                    </div>
                </PanelRow>
            )}
            {attributes.bgImg?.full?.url && !attributes.useFeaturedImage && !!isEnabled && (
                <PanelRow>
                    <div style={{ width: '100%' }}>
                        <CustomSelectControl
                            __next40pxDefaultSize
            ...
            {hasImage && !!isEnabled && !isImg && !attributes?.useParallax && !attributes?.bgVideo && !limited && (
                <PanelRow>
                    <div style={{ width: '100%' }}>
                        <p
                            style={{
            ...
            {hasImage && !!isEnabled && !attributes?.useParallax && !attributes?.bgVideo && !limited && (
                <PanelRow>
                    <div style={{ width: '100%' }}>
                        <SelectControl
                            label="Display Size"
            ...
            {hasImage && !!isEnabled && settings?.bgDisplaySize === 'custom' && !isImg && !attributes?.useParallax && !attributes?.bgVideo && !limited && (
                <PanelRow>
                    <Flex>
                        <FlexItem>
                            <UnitControl
            ...
            {!!isEnabled && !isImg && !limited && (
                <PanelRow>
                    <div style={{ width: '100%', marginTop: '8px' }}>
                        <CheckboxControl
                            label="Use Parallax Effect"
                            checked={jarallaxIsEnabled}
                            help="If enabled, Jarallax will be used to add parallax scrolling. Automatically enabled for background video."
                            onChange={(value) => setAttributes({ useParallax: value })}
                        />
                    </div>
                </PanelRow>
            )}
            {!!includeVideo && !isImg && !!isEnabled && !limited && (
                <PanelRow>
                    <TextControl
