...
export default function MethodFlexControls({
    ...
    const resetAll = () => {
        const keysToRemove = ['flexGrow', 'flexShrink', 'flexBasis', 'order'];
        ...
        <div className="method-control-set" style={{ width: '100%' }}>
            <ToolsPanel label={label} resetAll={resetAll}>
                ...
                {include.includes('order') && (
                    <ToolsPanelItem
                        label="Order"
                        hasValue={() => settings.order !== undefined}
                        onDeselect={() => updateSettings('order', undefined, true)}
                        onSelect={() => updateSettings('order', '')}
                    >
                        <TextControl
                            label="Order"
                            type="number"
                            value={settings?.order}
                            onChange={(value) => updateSettings('order', value)}
                        />
                    </ToolsPanelItem>
                )}
            </ToolsPanel>
