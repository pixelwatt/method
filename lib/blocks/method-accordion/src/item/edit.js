...
export default function Edit({ attributes, setAttributes, clientId, context }) {
    ...
    useEffect(() => {
        if (context?.['method-accordion/closed'] !== undefined) {
            setAttributes({ closed: context['method-accordion/closed'] });
        } else {
            setAttributes({ closed: false });
        }
    }, [context]);

    const { blockIndex, parentAccordionId, parentClientId, openItem, H } = useSelect(
        (select) => {
