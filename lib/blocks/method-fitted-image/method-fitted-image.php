...
function render_method_fitted_image_block( $block_attributes, $block ) {
    ...
    if ( $chosenImgId && method_check_array_key( $block_attributes, 'lightbox' ) ) {
        $lightboxAttrs = method_get_lightbox_attributes( $chosenImgId );
    }
    ...
    // trigger (an <a> to the full-size image, enhanced by the shared lightbox
    // script bundled into build/frontend.js — see lib/blocks/utils/lightbox.js).
    if ( $lightboxAttrs ) {
