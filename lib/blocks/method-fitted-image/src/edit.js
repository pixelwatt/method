...
export default function Edit({ attributes, setAttributes, clientId }) {
    ...
    const blockMarkup = (
        <Fragment>
            {!attributes?.linkToPost && !attributes?.lightbox && (
                <MethodLinkToolbar
        ...
        <>
            <InspectorControls>
                <PanelBody title="Image Options">
                    {!attributes?.linkToPost && !attributes?.link?.url && (
                        <div style={{ width: '100%', marginBottom: '12px' }}>
                            <CheckboxControl
                                label="Enable Lightbox"
                                checked={attributes.lightbox || false}
                                onChange={(value) => setAttributes({ lightbox: value })}
                                help="If checked, clicking or pressing the this imagr will open an uncropped version in a lightbox."
                            />
                        </div>
                    )}
                    <div style={{ width: '100%', marginBottom: '12px' }}>
                        <CheckboxControl
                            ...
                            onChange={(value) => setAttributes({ useFeaturedImage: value, ...(value ? {} : { linkToPost: false }) })}
                        />
                    </div>
                    {!!attributes?.useFeaturedImage && !attributes?.lightbox && !attributes?.link?.url && (
                        <div style={{ width: '100%', marginBottom: '12px' }}>
                            <CheckboxControl
                                label="Link to Post"
