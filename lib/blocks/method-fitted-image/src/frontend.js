/**
 * Method Fitted Image — Frontend
 *
 * Lightbox for fitted image blocks with the `lightbox` attribute enabled.
 *
 * The PHP render wraps the block in an <a> that points at the full-size,
 * uncropped image and marks it with `data-method-lightbox`, so without this
 * script (or without <dialog> support) the block is simply a link to the
 * image. This script intercepts those clicks and shows the image in a single
 * shared native <dialog> opened with showModal(), which provides focus
 * trapping, Escape-to-close, inert page content and focus restoration.
 */
( function () {
	'use strict';
