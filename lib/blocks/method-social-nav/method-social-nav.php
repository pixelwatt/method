...
function method_social_nav_enqueue_block_assets() {
    $option_key = apply_filters( 'method_utility_option_key', 'method_options');
    $theme_options = get_option( $option_key );
    wp_enqueue_script('method-social-nav-block-editor-script');
    wp_localize_script('method-social-nav-block-editor-script', 'socialNavData', array(
        'socialConfig' => $theme_options['social_accounts'] ?? array() // Replace 'my_key' with your array key
    ));
}
add_action('enqueue_block_editor_assets', 'method_social_nav_enqueue_block_assets');


function method_render_social_nav_block( $block_attributes, $block ) {
    $theme_options = get_option('method_options');
    $inlined = method_get_block_inline_styles( $block_attributes );
    $wrapper_attributes = get_block_wrapper_attributes();
    $output = '';
    if ( is_array( $theme_options ) ) {
        if ( array_key_exists( 'social_accounts', $theme_options ) ) {
            if ( is_array( $theme_options['social_accounts'] ) ) {
                if ( 0 < count( $theme_options['social_accounts'] ) ) {
                    $output = '<div ' . get_block_wrapper_attributes( ['class' => 'method-social-nav'] ) . '><ul class="method-sn">';
                    foreach( $theme_options['social_accounts'] as $item ) {
                        if ( ( method_check_array_key( $item, 'service' ) ) && ( method_check_array_key( $item, 'url' ) ) ) {
                            $platformIcon = '';
                            $platformName = '';
                            switch ($item['service']) {
                                case 'facebook':
                                    ...
                                    break;
                                case 'twitter':
                                    $platformIcon = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-twitter-x" viewBox="0 0 16 16"><path d="M12.6.75h2.454l-5.36 6.142L16 15.25h-4.937l-3.867-5.07-4.425 5.07H.316l5.733-6.57L0 .75h5.063l3.495 4.633L12.601.75Zm-.86 13.028h1.36L4.323 2.145H2.865z"/></svg>';
                                ...
                                case 'twitch':
                                    ...
                                    break;
                                case 'tiktok':
                                    $platformIcon = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-tiktok" viewBox="0 0 16 16"><path d="M9 0h1.98c.144.715.54 1.617 1.235 2.512C12.895 3.389 13.797 4 15 4v2c-1.753 0-3.07-.814-4-1.829V11a5 5 0 1 1-5-5v2a3 3 0 1 0 3 3z"/></svg>';
                                    $platformName = 'TikTok';
                                    break;
                                case 'threads':
                                    ...
                                    break;
                                case 'bluesky':
                                    $platformIcon = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-square-fill" viewBox="0 0 16 16"><path d="M0 2a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2z"/></svg>';
                            ...
                            $output .= '<li class="method-sn-i"><a href="' . $item['url'] . '" class="method-sn-i-l" target="_blank" rel="noopener noreferrer">' . $platformIcon . '<span class="visually-hidden">' . $platformName . '</span></a></li>';
                        }
                    }
                    $output .= '</ul></div>';
                }
