...
export default function Edit({ attributes, setAttributes, clientId }) {
    const blockProps = useBlockProps({ className: `method-social-nav` }, {});
    const socialLinks = (socialNavData.socialConfig || []).map((item, index) => {
        if (!item.service || !item.url) return null; // Skip invalid entries
        ...
        switch (item.service) {
            ...
            case "issuu":
                ...
                break;
            default:
                platformIcon = ''; // Default icon
                platformName = "Link";
        }

        return (
            <li className="crp-sn-i">
                <a className="crp-sn-i-l" target="_blank" rel="noopener noreferrer">
                    {platformIcon}<span className="visually-hidden">{platformName}</span>
                </a>
            </li>
        );
    });
    return (
        <>
            <div {...blockProps}><ul className='method-sn'>
                {socialLinks}
            </ul></div>
        </>
    );
